...
import { normalizeVocabulary, paginateVocabulary, Captions } from "@fluent-stack/shared";
  ...
  useEffect(() => {
    getAudioDurationInSeconds(SHARED_AUDIO_URL)
      .then((seconds) => {
        ...
        const pages = paginateVocabulary(normalizedVocab, ITEMS_PER_PAGE);
        const vocabDuration = pages.length * FRAMES_PER_PAGE;
      ...
      .catch((err: unknown) => {
        ...
        const normalizedVocab = normalizeVocabulary(SHARED_VOCABULARY);
        const pages = paginateVocabulary(normalizedVocab, ITEMS_PER_PAGE);
        setDurationInFrames(audioDurationFrames + pages.length * FRAMES_PER_PAGE);
      });
  }, []);
